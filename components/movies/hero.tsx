import { MovieDetails, WatchProviderID, TMDbConfiguration } from "@/lib/type/types";
import React from "react";
import "./hero.css";
import { formatFloat } from "@/lib/utils";
import { StarIcon } from "lucide-react";

interface HeroMovieAppProps {
  movie: MovieDetails;
  config: TMDbConfiguration;
  watchProvider: WatchProviderID;
}

export default function HeroMovieApp({ movie, config, watchProvider }: HeroMovieAppProps) {
  ...
  const posterUrl = `${baseUrl}${posterSize}${movie?.poster_path}`;
  const backdropUrl = `${baseUrl}${backdropSize}${movie?.backdrop_path}`;
  ...
  const tagline = movie?.tagline || "Unknown Tagline";
  const releaseYear = movie?.release_date?.slice(0, 4);

  return (
    <section id="heromovie">
      <div className="w-full h-[595px] relative bg-cyan-800">
        ...
        <div id="movieinfo" className="w-full bottom-0 absolute">
          <div className="container mx-auto px-4 sm:px-0 flex flex-col items-center ">
            <div
              id="posterimage"
              className="group/posterpath w-44 sm:w-60 rounded-md overflow-hidden relative"
            >
            ...
            <h1 className="text-xl font-semibold text-center mt-4">
              {movie.title} <span className="font-normal opacity-80">({releaseYear})</span>
            </h1>
