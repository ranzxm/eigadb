import { TMDbConfiguration, TVShow } from "@/lib/type/types";
...
export default function Heading({ title, totalResults, randomTV, config }: HeadingGenreProps) {
  const baseUrl = config.images.base_url;
  const backdropSize = config.images.backdrop_sizes[2];
  const randomTVUrl =
    randomTV.backdrop_path && `${baseUrl}${backdropSize}${randomTV.backdrop_path}`;
