...
interface ResultTVListProps {
  tvResponse: TVShowResponse;
  config: TMDbConfiguration;
  currentPage: number;
  slug: string;
}

export default function ResultTVList({ tvResponse, config, currentPage, slug }: ResultTVListProps) {
  const tv = tvResponse.results;
    ...
    <section id="searchResults">
      <div className="container mx-auto px-4 sm:px-0 py-4">
            ...
            >
              ...
              <div className="w-full h-full p-2 sm:p-4 absolute top-0 flex gap-2 sm:gap-4">
                <div id="poster" className="flex-none">
                  <a href={`/tv/${item.id}`} className="h-full">
                    <img
                      src={
                        item.poster_path
        ...
        <div className="w-full py-4 flex justify-center">
          <Pagination currentPage={currentPage} tv={tvResponse} slug={slug} />
        </div>
      </div>
    </section>
  );
